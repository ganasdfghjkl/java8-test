...
> * 它是针对流中的每一个元素都会执行action。
> * 这是一个终止操作。
> * 这个操作的行为是不确定的。对于并行流来说，这个操作并不会保证遵循流当中元素的顺序，如果去按照顺序去做的话，
>   就失去了并行流的优势；对于其中的一个元素，会在任意的一个时间，任意的一个线程中去执行。如果这个动作访问了
>   一个共享的状态，那么它就需要提供所需要的synchronization(这个动作访问了同步状态，就需要提供完整的同步机制)。
>   
>  
>  
#Collection 
...
>   * sourceSpliterator：源分割迭代器,只对头阶段有效；在这个管道被消费之前，如果他是非空的话，
>     那么```sourceSupplier```就一定是空的。在这个管道被消费之后，如果它为非空的话，就将它置为空。
>     
>   * sourceSupplier: 只对头管道有效；在这个管道被消费之前，为非空的话，那么```sourceSplitertor```就必须为空；
>     在消费之后，如果它不为空，将它置为空。
>   * sourceStage：指向pipeline的头阶段，如果为头阶段的时候为自身。
>   * depth：串行时在当前管道对象到pipeline源的中间操作个数，并行时为有状态的操作个数;在流管道准备计算的时候是有效的(准备工作时)。
>   * parallel：是否为并行的。
> 
> # Sink<E_IN> opWrapSink(int flags,Sink<E_OUT> sink) 将多个Sink进行拼接, 包装
>   * 接收一个Sink,这个Sink会接收这个操作的结果类型的元素， 并且会返回一个Sink,且返回的这个Sink,会接收这个操作的输入类型的元素；
>     而且会执行这个操作，然后将结果传递给提供的Sink。 
